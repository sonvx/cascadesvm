...
public class svm {
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target, ArrayList<Double>[] probs)
	{
		int i;
		...
		for(i=0;i<nr_fold;i++)
		{
			...
			ArrayList<Double> labelarray = new ArrayList<Double>(2);
			for(int o = 0 ; o < submodel.label.length ; o++)
				labelarray.add((double) submodel.label[o]);
			probs[0] = labelarray;
			    ...
			    param.svm_type == svm_parameter.NU_SVC))
			{
				...
				for(j=begin;j<end;j++) {
					...
					ArrayList<Double> probarray = new ArrayList<Double>(3);
					for(int o = 0 ; o < prob_estimates.length ; o++) {
						probarray.add(prob_estimates[o]);
					}
					probarray.add((double)(i+1));	//add the fold index
					probs[perm[j]+1] = probarray;
				}
				
				//for(int z = begin ; z < end ; z++)
				//	System.out.println(perm[z]+"-" +probs[perm[z]+1]);
			}
			else
				for(j=begin;j<end;j++) {
