...
public class KernelChunkTranslator {
	 ...
	 * @throws IOException
	 */
	public static void translateMinus(File in, File outfile, int dim) throws IOException {
		BufferedWriter bw = new BufferedWriter(new FileWriter(outfile),1024*8*16);
		DataInputStream br = new DataInputStream(new FileInputStream(in));
		while(br.available() > 0) {
			for(int i = 0 ; i < dim -1; i ++) {
	...
	public static void main(String[] args) throws Exception {
		...
		translateMinus(new File(args[0]), new File(args[1]), Integer.parseInt(args[2]));
